...
| `evals/run.ts` | The orchestrator that gates CI: knobs, the case loop, the report, five exit codes. Since 5.4b it decides nothing — the populations, the verdicts and the metrics all come from `evals/evaluators/` |
| `evals/evaluators/` | Pure functions over cases and results, verdicts computed in code. `partition.ts` is the load-bearing one: who counts toward which denominator, decided ONCE (parked, ERROR and injection cases are excluded for three different reasons). Then `case-verdict.ts` (shared by both targets, so a verdict means the same thing on each), `recall.ts` (run 1 AND every-run), `coverage.ts`, `guardrails.ts` (which LAYER held — planner, threshold or prompt), `injection.ts`, `refusal.ts` (false refusals), `github-answer.ts`, `github-process.ts`, and `faithfulness.ts` — the judge (opt-in), verdict computed in code, not by the model. `index.ts` is a descriptor table, not a dispatcher, and every entry names the `METRICS` id it is stored under so a number that cannot be diffed is findable |
| `evals/targets/` | `in-process.ts` (the pipeline in this process: retrieval ONCE per case, N generations), `agent-service.ts` (the Python service's HTTP client plus the `query_log` and Langfuse readers), `agent-run.ts` (that target's case loop and its session — the two thread lists are kept apart so the suite's cost figure stays a series), and `langfuse-api.ts`. `index.ts` says what each target can and cannot measure, because `recall` means run 1 on both and `recallEveryRun` is a question only the service can be asked |
