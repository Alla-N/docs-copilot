...
--
-- The eval set has 9 cases I invented. Production has questions I never imagined, and the
-- only way a false refusal enters the eval set is if something notices it. This is that
-- something.
...
create table if not exists query_log (
    ...
    retrieval_mode text        not null,   -- 'reranked' | 'cosine-fallback'
    latency_ms     int
);
...
-- Sanity check on the health of the whole system, over real traffic rather than 9 cases:
